  ...
  const fetchAllArtistsContent = async (mode) => {
    ...
    try {
      ...
      if (mode === 'all-videos') {
        ...
        // NOTE: we do NOT use the embedded `artists(...)` select syntax here,
        // because that requires PostgREST to already know about a foreign key
        // relationship between music_videos.artist_id and artists.id. If that
        // relationship isn't registered, the whole query throws and the page
        // silently renders nothing. Instead we fetch videos and artists
        // separately and merge them in JS — this always works.
        ...
        if (videos && videos.length > 0) {
          const artistIds = [...new Set(videos.map(v => v.artist_id).filter(Boolean))];
          let artistsById = {};
          ...
          if (artistIds.length > 0) {
            ...
            if (artistsErr) {
              console.error('Artists lookup error (videos):', artistsErr);
            } else {
              artistsById = Object.fromEntries((artistsData || []).map(a => [a.id, a]));
            }
          }
          ...
          tracks = videos.map(video => ({
            ...
            artist: artistsById[video.artist_id]?.name || 'Unknown Artist',
        ...
        // Albums live in the `albums` table — bioscope_content is Deezer-scraped
        // filler data and was never the source of truth for albums (its
        // `content_type` column is always null, so filtering on it returned 0
        // rows every time). Query `albums` directly, same FK-independent
        ...
        if (albumsError) {
          console.error('Albums fetch error:', albumsError);
          throw albumsError;
        }
        ...
        if (albumsData && albumsData.length > 0) {
          ...
          if (artistIds.length > 0) {
            ...
            if (artistsErr) {
              console.error('Artists lookup error (albums):', artistsErr);
            } else {
              artistsById = Object.fromEntries((artistsData || []).map(a => [a.id, a]));
            }
          }

          tracks = albumsData.map(album => ({
            id: album.id,
            title: album.title || 'Untitled Album',
            artist: artistsById[album.artist_id]?.name || 'Unknown Artist',
            artist_id: album.artist_id,
            album: album.title || 'Album',
            duration: 0,
            preview: '',
            cover_small: album.cover_small || album.cover_image || 'https://via.placeholder.com/150x150/8B6914/f4d090?text=' + encodeURIComponent(album.title || 'Album'),
            cover_medium: album.cover_medium || album.cover_image || 'https://via.placeholder.com/300x300/8B6914/f4d090?text=' + encodeURIComponent(album.title || 'Album'),
            isAlbum: true,
            albumId: album.id,
